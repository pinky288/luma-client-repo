...
const Roadmap = () => {
  // ১. স্ক্রল এনিমেশন সেটআপ
  const containerRef = React.useRef(null);
  const { scrollYProgress } = useScroll({
    target: containerRef,
    offset: ["start center", "end end"] // এনিমেশন শুরু এবং শেষ হওয়ার পয়েন্ট
  });
  ...
  return (
    <section ref={containerRef} className="py-24 bg-black px-6">
      <div className="max-w-7xl mx-auto flex flex-col items-center">
        ...
        {/* Title Block (Centered, Normal Font) */}
          ...
          <div className="space-y-16">
            {steps.map((step, index) => (
              <motion.div 
                ...
                viewport={{ once: true, amount: 0.5 }} // ৫০% স্ক্রিনে আসলে এনিমেট হবে
                transition={{ delay: index * 0.1, duration: 0.5, ease: 'easeOut' }}
                className={`relative flex items-center w-full ${
                  index % 2 === 0 ? "md:flex-row-reverse" : "md:flex-row"
                }`}
              >
                {/* 1. Glassmorphism Card (Black BG + Green Border/Glow) */}
                ...
                {/* 2. Center Node (Pulsing Dot on Green Line) */}
